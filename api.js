...
async function updateCartItem(cartId, quantity) {
  ...
  const response = await axios.patch(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/carts`, {data});
  return response.data;
}
...
async function createOrder(userInfo) {
  ...
  const response = await axios.post(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/orders`, {data});
  return response.data;
}
...
async function updateOrderStatus(orderId, isPaid) {
  ...
  const response = await axios.put(`${BASE_URL}/api/livejs/v1/admin/${API_PATH}/orders`, 
    {data}, {headers: {authorization: ADMIN_TOKEN}});
  return response.data;
}
